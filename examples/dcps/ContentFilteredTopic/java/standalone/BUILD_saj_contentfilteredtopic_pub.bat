...
set MAINCLASS=ContentFilteredTopicDataPublisher
set JARFILE=saj_contentfilteredtopic_pub.jar
set MANIFEST=saj_contentfilteredtopic_pub.manifest
...
del /f/s/q classes\ContentFilteredTopicDataPublisher.class classes\DDSEntityManager.class classes\ErrorHandler.class 2>nul
...
echo javac -sourcepath ..\src -cp "saj_contentfilteredtopic_types.jar;classes\;..\..\..\..\..\jar\dcpssaj.jar;" -d classes\ ..\src\ContentFilteredTopicDataPublisher.java
javac -sourcepath ..\src -cp "saj_contentfilteredtopic_types.jar;classes\;..\..\..\..\..\jar\dcpssaj.jar;" -d classes\ ..\src\ContentFilteredTopicDataPublisher.java
IF NOT %ERRORLEVEL% == 0 (
  ECHO:
  ECHO *** Java compilation of ..\src\ContentFilteredTopicDataPublisher.java failed
  ECHO:
  GOTO error
)
...
echo pushd classes\ ^& jar %JARFLAGS% %JARFILE% %MANIFEST%  ContentFilteredTopicDataPublisher.class DDSEntityManager.class ErrorHandler.class ^& popd
pushd classes\ & jar %JARFLAGS% %JARFILE% %MANIFEST%  ContentFilteredTopicDataPublisher.class DDSEntityManager.class ErrorHandler.class & popd
